...
export default function Admin() {
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch('/api/admin/message', {
        ...
        body: JSON.stringify({ toEmail: recipient.email, toName: recipient.name, trackingNumber: selectedTracking.trackingNumber, subject: messageData.subject, message: messageData.message, senderName: messageData.senderName || 'Global Track Logistics Team' }),
      });
      const data = await response.json();
  ...
  if (isLoading) return <div className="min-h-screen flex items-center justify-center bg-gray-100"><div className="animate-spin rounded-full h-12 w-12 border-b-2 border-[#14532d]"></div></div>;
      ...
      <div className="max-w-7xl mx-auto p-6">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
          ...
          <div className="flex gap-3">
            <button onClick={() => setShowForm(true)} className="flex items-center gap-2 px-5 py-2.5 bg-[#14532d] text-white rounded-lg hover:bg-[#166534] shadow-lg">
              <FaPlus /> New Shipment
            </button>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          {[{ label: 'Total Shipments', value: trackings.length }, { label: 'In Transit', value: trackings.filter(t => t.status === 'In Transit').length }, { label: 'Delivered', value: trackings.filter(t => t.status === 'Delivered').length }, { label: 'Pending', value: trackings.filter(t => t.status === 'Pending').length }].map((stat, i) => (
            <div key={i} className="bg-white dark:bg-gray-800 rounded-xl p-4 shadow-sm">
              <p className="text-2xl font-bold text-[#14532d] dark:text-[#22c55e]">{stat.value}</p>
        ...
        <div className="mb-6">
          <div className="relative">
            ...
            <input type="text" placeholder="Search by tracking number, shipper or receiver..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full pl-12 pr-4 py-3 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-[#22c55e] focus:border-transparent" />
          </div>
        </div>

        {/* Shipments List - Responsive Tiles */}
          ...
          {filteredTrackings.length === 0 ? (
            ...
            <div key={tracking._id} className="bg-white dark:bg-gray-800 rounded-xl p-4 shadow-sm">
              <div className="flex justify-between items-start mb-3">
                <div>
                  <p className="font-bold text-[#14532d] dark:text-[#22c55e] text-lg">{tracking.trackingNumber}</p>
              ...
              <div className="flex flex-wrap gap-2 pt-3 border-t border-gray-100 dark:border-gray-700">
                <button onClick={() => openUpdateModal(tracking)} className="flex-1 min-w-[80px] px-3 py-2 bg-green-100 text-green-700 rounded-lg text-xs font-medium hover:bg-green-200 flex items-center justify-center gap-1"><FaEdit size={12} /> Update</button>
      ...
      {showForm && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-2 sm:p-4 overflow-y-auto">
          <div className="bg-white dark:bg-gray-800 rounded-2xl w-full max-w-4xl max-h-[95vh] my-2 overflow-hidden shadow-2xl">
            <div className="bg-gradient-to-r from-[#14532d] to-[#166534] px-4 md:px-6 py-3 md:py-4 flex justify-between items-center sticky top-0 z-10">
              ...
              <form onSubmit={handleCreate} className="space-y-4 md:space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                  <div className="space-y-3 md:space-y-4">
                    <h3 className="font-semibold text-gray-900 dark:text-white flex items-center gap-2 text-sm md:text-base"><FaUser className="text-[#22c55e]" /> Shipper Information</h3>
                  ...
                  <div className="space-y-3 md:space-y-4">
                    <h3 className="font-semibold text-gray-900 dark:text-white flex items-center gap-2 text-sm md:text-base"><FaBox className="text-[#22c55e]" /> Receiver Information</h3>
                ...
                <div className="bg-gray-50 dark:bg-gray-900/50 rounded-xl p-3 md:p-5">
                  <h3 className="font-semibold text-gray-900 dark:text-white mb-3 md:mb-4 flex items-center gap-2 text-sm md:text-base"><FaArrowRight className="text-[#22c55e]" /> Shipment Information</h3>
                ...
                <div className="bg-gray-50 dark:bg-gray-900/50 rounded-xl p-5">
                  ...
                  <div className="mt-4">
                    ...
                    <input type="number" min="0" placeholder="0" value={formData.totalFreight} onChange={(e) => setFormData({...formData, totalFreight: e.target.value})} className="w-full px-4 py-3 rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-[#22c55e]" />
                  </div>
                </div>
                ...
                <div className="flex justify-end gap-4 pt-4 border-t">
                  ...
                  <button type="submit" disabled={isSubmitting} className="px-8 py-3 bg-[#14532d] text-white font-semibold rounded-lg hover:bg-[#166534] disabled:opacity-50 flex items-center gap-2">
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-2 sm:p-4">
          <div className="bg-white dark:bg-gray-800 rounded-2xl p-4 md:p-6 w-full max-w-lg shadow-2xl">
            {sendSuccess ? (
              ...
              <>
                <div className="flex justify-between items-center mb-4 md:mb-6"><h2 className="text-lg md:text-xl font-bold text-gray-900 dark:text-white flex items-center gap-2"><FaEnvelope className="text-[#22c55e]" /> Send Message</h2><button onClick={() => setShowMessageModal(false)} className="text-gray-500 hover:text-gray-700 dark:text-gray-400"><FaTimes size={20} /></button></div>
                <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-3 md:p-4 mb-4">
                  ...
                  <p className="text-xs md:text-sm text-gray-500">Ref: <span className="font-bold text-[#14532d] dark:text-[#22c55e]">{selectedTracking.trackingNumber}</span></p>
                </div>
                <form onSubmit={handleSendMessage} className="space-y-3 md:space-y-4">
                  ...
                  <div className="flex flex-col-reverse sm:flex-row justify-end gap-2 md:gap-3 pt-2"><button type="button" onClick={() => setShowMessageModal(false)} className="px-4 md:px-5 py-2 border border-gray-300 text-gray-700 dark:border-gray-600 dark:text-gray-300 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 text-sm md:text-base">Cancel</button><button type="submit" disabled={isSending} className="px-4 md:px-5 py-2 bg-[#14532d] text-white rounded-lg disabled:opacity-50 flex items-center justify-center gap-2 text-sm md:text-base">{isSending ? <><FaSpinner className="animate-spin" /> Sending...</> : <><FaEnvelope /> Send</>}</button></div>
                </form>
              </>
            )}
          </div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl p-4 md:p-6 w-full max-w-lg shadow-2xl">
            <div className="flex justify-between items-center mb-4 md:mb-6">
              <h2 className="text-lg md:text-xl font-bold text-gray-900 dark:text-white flex items-center gap-2">
                <FaEdit className="text-[#22c55e]" /> Update Shipment Status
              </h2>
            ...
            <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-3 md:p-4 mb-4">
              <p className="text-xs md:text-sm text-gray-500">Tracking: <span className="font-bold text-[#14532d] dark:text-[#22c55e]">{selectedTracking.trackingNumber}</span></p>
            ...
            <form onSubmit={handleUpdateStatus} className="space-y-3 md:space-y-4">
              ...
              <div>
                <label className="block text-xs md:text-sm font-medium text-gray-700 dark:text-gray-300 mb-1 flex items-center gap-1">
                  <FaMapMarkerAlt className="text-[#22c55e]" /> Current Location *
                </label>
              ...
              <div className="flex flex-col-reverse sm:flex-row justify-end gap-2 md:gap-3 pt-3 md:pt-4 border-t">
                ...
                <button type="submit" disabled={isUpdating} className="px-4 md:px-5 py-2 bg-green-600 text-white rounded-lg disabled:opacity-50 flex items-center justify-center gap-2 text-sm md:text-base hover:bg-green-700">
            ...
            <div className="p-4 md:p-6 max-h-[calc(100vh-160px)] overflow-y-auto">
              <form onSubmit={handleEditShipment} className="space-y-4 md:space-y-6">
                <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-3 md:p-4">
                  <p className="text-sm text-gray-500">Tracking: <span className="font-bold text-[#14532d] dark:text-[#22c55e]">{selectedTracking.trackingNumber}</span></p>
                </div>
                  ...
                  <div className="md:col-span-2">
                    ...
                    <div className="flex items-center gap-3 bg-gray-50 dark:bg-gray-700 rounded-lg p-3">
                      <button
                        ...
                        className={`relative inline-flex h-8 w-14 items-center rounded-full transition-colors duration-200 ${editData.showLiveMap ? 'bg-green-600' : 'bg-gray-300 dark:bg-gray-600'}`}
                      >
